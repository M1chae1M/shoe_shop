...
export default class Boot extends Component{
  state={
    data:{},
    // sizeState:this.state?.data?.sizes.split(',')[0]||'0',
    howMany:1,
  }
  // componentDidMount(){
    // fetch(`${process.env.NEXT_PUBLIC_API_URL}singleProduct`,{
    // fetch(`/api/singleProduct`,{
    //   method:'POST',
    //   headers:{'Content-Type':'application/json'},
    //   body:JSON.stringify({id:this.props.boot})
    // })
    // .then(resp=>resp.json())
    // .then(resp=>resp.resp)
  ...
  render(){
    ...
    //   Router.push('/');
    //   return {boot,name,image,price,howMany,sizeState,time:new Date().getTime()}
    // }
